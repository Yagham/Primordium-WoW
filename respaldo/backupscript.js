...
async function renderCart() {
  ...
  cart.forEach(item => {
    ...
    tr.innerHTML = `
      <td>
        <a
          href="https://www.wowhead.com/wotlk/es/item=${prod.wowheadId}"
          class="wowhead"
          target="_blank"
        >
          <img
...
document.addEventListener('DOMContentLoaded', () => {
        ...
        products.forEach(p => {
            ...
            col.innerHTML = `
                <div class="card h-100 text-center">
                    <a href="https://www.wowhead.com/wotlk/es/item=${p.wowheadId}" class="wowhead">
    ...
    function selectTypeSize(size) {
        ...
        const types = categoriesData[currentCategory][currentSubcat][size];
        ...
        types.forEach(tp => {
            ...
            btn.onclick = () => {
                // 1) Quitar active de todos los botones de nivel 4
                document
                    .querySelectorAll('#shop-type .btn')
                    .forEach(b => b.classList.remove('active'));
                // 2) Marcar este como activo
                btn.classList.add('active');
                // 3) Guardar el tipo y mostrar productos
                currentType = tp;
                renderProducts();
            };
            el.appendChild(btn);
